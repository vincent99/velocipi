...
export declare interface Expander {
  on(event: 'change', listener: (neu: number, old: number) => void): this;
}
...
export class Expander extends EventEmitter {
  ...
  public async init(inputs = 0xffff) {
    ...
    this.timer = setInterval(async () => {
      ...
      if (res.changed) {
        // logger.debug(
        //   `Changed: ${format(res.previous ^ res.value)
        //     .replace(/0/g, '.')
        //     .replace(
        //       /1/g,
        //       'X'
        //     )} -> ${format(res.previous)} -> ${format(res.value)}`
        // );
        this.emit('change', res.value, res.previous);
      }
  ...
  public async getInterruptConfig(): Promise<{
    enabled: number;
    mode: number;
    value: number;
  }> {
...
export function format(val: number) {
  return (
    strPad(`${(val >> 8).toString(2)}`, 8, '0', false) +
    ' ' +
    strPad(`${(val & 0xff).toString(2)}`, 8, '0', false)
  );
}
