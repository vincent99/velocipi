...
const props = withDefaults(defineProps<{ showLed?: boolean }>(), { showLed: true });
...
<template>
  <div class="screen-wrap" :style="{ width: w + 'px', height: h + 'px' }">
    ...
    <div v-if="props.showLed && ledMode !== 'off'" class="led-dot" :class="ledMode" :style="blinkStyle" />
  </div>
</template>
